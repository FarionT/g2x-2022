...
class User { 
    ...
    function isVote($data = array(), $gameid){
        $checkQuery = "SELECT user_id FROM ".$this->voteTbl." WHERE user_id = '".$data['oauth_uid']."' AND gameid = '".$gameid."'";
        $checkResult = mysqli_query($this->db, $checkQuery);
        return !empty(mysqli_fetch_assoc($checkResult));
    }
    function DoVote($data = array(), $gameid){
